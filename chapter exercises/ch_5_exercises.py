...
def joke():
    input("What is a programmer's favorite pet? (press enter to see the answer)")
    print("A Python!")
...
def main():
    input("press enter to see each line of the knock, knock joke.")
    knock()
    who()
    who_it_is()
    who_it_is_who()
...
def punchline():
    print("Didn't know you were going to be this excited to see me!")
...
def main5():
    r = random.randrange(1, 11)  # generate a random integer between 1-10, assign it to the variable r
    r2 = r * r
    area(r2)
    
    
def area(radius_squared):
    my_area = PI * radius_squared  # insert the PI constant multiplied by the parameter here
...
def main6():
    ...
    # call the bmi function and assign the result to a variable named answer
    answer = bmi(height, weight)

    # print the variable answer, make sure to format it to 1 decimal place
...
def bmi(height_inches, weight_pounds):
    ...
    # send the patient_bmi value back to main6
    return patient_bmi
